...
pub async fn github_callback(
    ...
    let mut user_info: GitHubUserInfo = user_info_response
        .json()
        .await
        .map_err(|e| actix_web::error::ErrorInternalServerError(format!("Failed to parse user info: {}", e)))?;
    ...
    if user_info.email.is_none() {
        let email_response = client
            ...
            .send()
            .await
            .map_err(|e| actix_web::error::ErrorInternalServerError(format!("Failed to get user emails: {}", e)))?;
        ...
        let emails: Vec<GitHubEmail> = email_response
            .json()
            .await
            .map_err(|e| actix_web::error::ErrorInternalServerError(format!("Failed to parse user emails: {}", e)))?;
