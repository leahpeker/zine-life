import { J as Da, M as il } from './DPuKpAsp.js';
import { _ as Va } from './D88yL9vU.js';
var fn = {},
	cn = {},
	cr,
	vn;
		...
		(!v || !f) &&
			...
			c
				...
				: d && d.now
					...
					: y && !h
						? ((P = new y()), (_ = P.port2), (P.port1.onmessage = I), (O = r(_.postMessage, _)))
						: n.addEventListener &&
							  t(n.postMessage) &&
							  !n.importScripts &&
							  E &&
			...
			try {
				W
					? ($ || (B.rejection === Ge && $e(B), (B.rejection = Ue)),
						W === !0 ? (Ee = G) : (Se && Se.enter(), (Ee = W(G)), Se && (Se.exit(), (hr = !0))),
						Ee === R.promise
		...
		Ve = function (R, B, G) {
			...
			z
				? (($ = V.createEvent('Event')),
					...
					t.dispatchEvent($))
				: ($ = { promise: B, reason: G }),
				!A && (W = t['on' + R]) ? W($) : R === H && y('Unhandled promise rejection', G);
		},
		Xe = function (R) {
			a(d, t, function () {
				var B = R.facade,
	...
	return (
		(Hi = function (f, g, d) {
			...
			if (m) E = f.iterator;
				...
				if (i(O)) {
					for (P = 0, _ = a(f); _ > P; P++) if (((A = C(f[P])), A && s(v, A))) return A;
					return new c(!1);
				}
				E = o(f, O);
			}
		...
		x &&
			(v = function (E) {
				var O = this,
					...
					U,
					L;
				if (A) return (A.lastIndex = O.lastIndex), (q = n(v, A, _)), (O.lastIndex = A.lastIndex), q;
				var V = P.groups,
					M = m && O.sticky,
					N = n(t, O),
					k = O.source,
...
`)) &&
					...
					b &&
						...
						C.length > 1 &&
						n(h, C[0], I, function () {
							for (w = 1; w < arguments.length - 2; w++) arguments[w] === void 0 && (C[w] = void 0);
						}),
					C && V)
				)
					for (C.groups = U = s(null), w = 0; w < V.length; w++) (L = V[w]), (U[L[0]] = C[L[1]]);
				return C;
			}),
		(aa = v),
		aa
				...
				return [
					...
					function (L, V) {
						...
						for (var Ge = '', fe = 0, Ne = 0; Ne < be.length; Ne++) {
							oe = be[Ne];
							for (
								var _e = h(oe[0]), me = b(x(u(oe.index), N.length), 0), ce = [], xe, De = 1;
								De < oe.length;
								De++
							)
								E(ce, _(oe[De]));
					...
					function (_, A) {
						...
						for (var M = 0, N = 0, k = []; N < I.length; ) {
							...
							if (z === null || (H = p(u(L.lastIndex + (g ? N : 0)), I.length)) === M)
								...
								for (var X = 1; X <= z.length - 1; X++) if ((y(k, z[X]), k.length === V)) return k;
								N = M = H;
							}
						}
						return y(k, m(I, M)), k;
...
function Af(n, e, r, t, i, a) {
	...
	for (var $e = 0; $e < t; $e++) {
		...
		var ve = s[b],
			...
			Ee = c * R;
		f = v;
		for (var lr = 0; lr < h; lr++) (f.r = ve), (f.g = ge), (f.b = Oe), (f.a = R), (f = f.next);
...
class ee {
	...
	static parsePath(e) {
		for (var r = ye(e), t = r.length, i = [], a = 0; a < t; a += 2) i.push(new ee(r[a], r[a + 1]));
		return i;
	}
	angleTo(e) {
		...
		if (!this.glyphInfo) {
			...
			(o === 'middle' || o === 'center') && (b = -p / 2),
				...
				t.forEach((x, S) => {
					...
					(b = E),
						!(!O.p0 || !O.p1) &&
							this.glyphInfo.push({ text: t[S], p0: O.p0, p1: O.p1, rotation: P });
				});
		}
	}
	parsePathData(e) {
