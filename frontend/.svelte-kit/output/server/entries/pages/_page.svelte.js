import { F as store_get, G as head, E as escape_html, I as ensure_array_like, J as attr, K as stringify, M as unsubscribe_stores, C as pop, z as push } from "../../chunks/index.js";
import "../../chunks/client.js";
import { a as authStore } from "../../chunks/auth.js";
import { H as Header, b as buildApiUrl, A as API_ENDPOINTS } from "../../chunks/Header.js";
import { U as UI_TEXT, C as CanvasThumbnail } from "../../chunks/CanvasThumbnail.js";
function _page($$payload, $$props) {
  ...
  Header($$payload, {});
  $$payload.out += `<!----> <main class="bg-black min-h-screen photocopied scan-lines relative" style="margin-top: 64px;">`;
  if (store_get($$store_subs ??= {}, "$authStore", authStore).user) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<section class="bg-black py-12 px-4"><div class="max-w-6xl mx-auto"><div class="mb-12"><h1 class="text-4xl md:text-6xl font-black text-white mb-4 tracking-wider transform -skew-x-6 font-punk">${escape_html(UI_TEXT.HEADINGS.WELCOME_BACK(store_get($$store_subs ??= {}, "$authStore", authStore).user.name))}</h1> <p class="text-xl text-green-400 font-mono">Ready to create more underground graphics?</p></div> <div class="flex flex-wrap gap-4 mb-12"><button class="bg-green-400 text-black px-8 py-3 text-xl font-black tracking-wide transform -skew-x-6 hover:bg-green-300 transition-colors border-2 border-green-400 font-industrial" aria-label="Create a new design in the editor">NEW DESIGN</button> <button class="bg-transparent text-green-400 px-8 py-3 text-xl font-black tracking-wide border-2 border-green-400 hover:bg-green-400 hover:text-black transition-colors font-industrial" aria-label="View all your designs in the dashboard">VIEW ALL DESIGNS</button></div> <div class="mb-12"><h2 class="text-3xl font-black text-white mb-6 transform -skew-x-3 font-punk border-b-2 border-gray-800 pb-2">YOUR RECENT WORK</h2> `;
    if (loading) {
      $$payload.out += "<!--[-->";
      $$payload.out += `<div class="text-green-400 font-mono">Loading your designs...</div>`;
    } else if (recentDesigns.length === 0) {
      $$payload.out += "<!--[1-->";
      $$payload.out += `<div class="text-center py-12 border-2 border-gray-800 bg-gray-900"><div class="text-green-400 text-6xl mb-4">[+]</div> <h3 class="text-xl font-black text-white mb-2 font-industrial">NO DESIGNS YET</h3> <p class="text-gray-400 font-mono mb-4">Time to create your first underground masterpiece</p> <button class="bg-green-400 text-black px-6 py-2 font-black tracking-wide transform -skew-x-6 hover:bg-green-300 transition-colors border-2 border-green-400 font-industrial" aria-label="Start creating your first design">START NOW</button></div>`;
    } else {
      $$payload.out += "<!--[!-->";
      ...
      for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
        ...
        $$payload.out += `<li class="border-2 border-gray-800 bg-gray-900 p-4 hover:border-green-400 transition-colors photocopied"><div class="h-40 mb-4 flex items-center justify-center bg-gray-800">`;
        CanvasThumbnail($$payload, {
          design,
          size: "medium",
          className: "bg-white"
        });
        $$payload.out += `<!----></div> <h3 class="text-lg font-black text-white mb-2 font-industrial">${escape_html(design.title)}</h3> <p class="text-gray-400 font-mono text-xs mb-3">${escape_html(new Date(design.updated_at).toLocaleDateString())}</p> <button class="bg-transparent text-green-400 px-4 py-1 text-sm font-black border border-green-400 hover:bg-green-400 hover:text-black transition-colors font-industrial"${attr("aria-label", `Edit ${stringify(design.title)}`)}>EDIT</button></li>`;
      }
      $$payload.out += `<!--]--></ul>`;
    }
