...
import {
	z as zineStore,
	h as elementRegistry,
	i as ElementCategories,
	j as StyleGuide,
	k as SaveStatus,
	l as SaveStatusText,
	U as UIStrings,
	m as SaveStatusColors,
	f as Circle,
	S as ShapeTypes,
	n as ElementTypes,
	e as Rect,
	...
	I as Image,
	H as Header,
	L as LayoutDimensions,
	b as buildApiUrl,
	A as API_ENDPOINTS
} from '../../../chunks/pageStore.js';
import 'clsx';
import 'konva';
import 'jspdf';
...
function updateElement(elementId, updates) {
	elements.update((current) =>
		current.map((element) => (element.id === elementId ? { ...element, ...updates } : element))
	);
}
...
function updateElementPosition(id, x, y) {
	elements.update((current) =>
		current.map((element) => (element.id === id ? { ...element, x, y } : element))
	);
}
...
function EditBarComponent($$payload, $$props) {
	push();
	let { selectedElement, isVisible, onUpdateElement } = $$props;
...
function CanvasEditPanel($$payload, $$props) {
	push();
	let { backgroundColor, showGrid, zoom, canvasWidth, canvasHeight, onUpdateBackground } = $$props;
	const zoomLevels = [25, 50, 75, 100, 125, 150, 200, 300, 400, 500];
	const availableZoomLevels = zoomLevels.includes(zoom)
		? zoomLevels
		: [...zoomLevels, zoom].sort((a, b) => a - b);
...
function MultiPageCanvas($$payload, $$props) {
	...
	function handleStageReady(stage, pageIndex) {
		...
		if (
			pageIndex === store_get(($$store_subs ??= {}), '$zineStore', zineStore).currentPageIndex &&
			onStageReady
		) {
			onStageReady(stage);
		}
	...
	function getElementsForPage(pageId) {
		const pageElements = store_get(($$store_subs ??= {}), '$elements', elements).filter(
	...
	const each_array = ensure_array_like(
		store_get(($$store_subs ??= {}), '$zineStore', zineStore).pages
	);
...
function EditorTopBar($$payload, $$props) {
	push();
	let { title, canUndo, canRedo, onSave, saveStatus = SaveStatus.IDLE } = $$props;
...
function _page($$payload, $$props) {
	...
	const selectedElement = selectedId
		? store_get(($$store_subs ??= {}), '$elements', elements).find(
